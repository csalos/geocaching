...
(function() {
	...
	// Létrehozzunk az új TABLE elemet
	const tableInner = '<tr><th colspan="4">Esemény ládák</th></tr>' + 
					   '<tr><th width="66px">Azonosító</th>' + 
						   '<th>Név</th>' + 
						   '<th width="75px">Mikor?</th>' + 
						   '<th width="100px">Hol?</th>' + 
					   '</tr><tr id="rowsEvent"></tr>';
	const eventTable = document.createElement('table');
	eventTable.id = 'event';
	eventTable.width = '100%';
	eventTable.height = '474px';
	eventTable.setAttribute("style", "white-space: pre");
	eventTable.innerHTML = tableInner;
	
	// Beszúrjuk a TABLE-et a script tag elé
	if (script && script.parentNode) {
    	script.parentNode.insertBefore(eventTable, script);
	}
